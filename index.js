...
client.on("ready", () => {
  ...
  client.user.setActivity(`Support Bot`);
});
...
client.on("message", async message => {
  ...
  if (message.channel.type !== "dm") {
      if (message.channel.name.indexOf('ticket') > -1 && command != "close") {
          ...
          if (command == "close" && message.channel.name.indexOf('ticket') > -1) {
              ...
              const embed = new RichEmbed()
                  .setColor(0xCF40FA)
                  .addField(`New message`, "Your ticket has been closed, no need to reply to this message")
                  .setTimestamp();
              pq.send({
                  embed: embed
              });
  ...
  if (message.channel.type === "dm") {
      ...
      server.createChannel(`ticket-${message.author.id}`, "text").then(c => {
          ...
          message.channel.send(`:white_check_mark: **__Your ticket has been forwarded to the staff.__**`);
          const embed = new RichEmbed()
              .setColor(0xCF40FA)
